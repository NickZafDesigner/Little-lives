...
import { applyNeedDeltas, clampNeed, NEED_DECAY, NEED_IDS } from "../data/needs";
import {
  AMBIENT_NPCS,
  ambientNpcById,
  isAmbientNpcId,
...
import { LOTS, lotAtTile } from "../world/lots";
...
interface NpcRuntime {
  ...
  path: GridPos[];
  waitUntil: number;
  /** Street hangabout — stays put, one-liner chat only. */
  ambient?: boolean;
}
...
export function createWorldScreen(
  ...
  const addWall = (tx: number, ty: number) => {
    ...
    const mesh = new THREE.Mesh(
      ...
      new THREE.MeshLambertMaterial({ color: 0xfdf3e0, flatShading: true }),
    );
  ...
  const walkSpeed = () =>
    state.needs.energy < 15 ? WALK_SPEED * 0.6 : WALK_SPEED;
  ...
  const openFurnitureMenu = (furn: PlacedFurniture, x: number, y: number) => {
    ...
    const petItems = new Set(["pet_bed", "pet_bowl", "toy_ball"]);
    ...
    menu.show(def.name, def.category, options, x, y, (id) => {
      if (id === "pick_flowers") {
        Audio.sfx("interact");
      ...
      delayed(interaction.durationMs, () => {
        state.needs = applyNeedDeltas(state.needs, interaction.needDeltas);
        ...
        Audio.sfx("success");
        state.showToast(`${interaction.label} — that's better!`);
        if (
  ...
  const openJobMenu = (jobId: string, x: number, y: number) => {
    ...
    const options: MenuOption[] = [
      {
        id: "shift",
        label: state.jobActive ? "Do a task" : "Clock in",
        sub: state.jobActive
          ? `${state.jobTasksDone}/${job.shiftTasks} tasks done`
          : `Full shift pays $${job.pay}`,
      },
    ];
    menu.show(job.name, lot?.name ?? "Work", options, x, y, () => {
      if (!state.jobActive) {
      ...
      Audio.sfx("interact");
      state.startBusy("Working", job.durationMs);
      delayed(job.durationMs, () => {
        ...
        if (state.jobTasksDone >= job.shiftTasks) {
          ...
          Audio.sfx("coin");
          Audio.sfx("success");
          state.showToast(`Shift complete! You earned $${job.pay}`);
          quests.emit(jobShiftEvent(job.id));
        } else {
          Audio.sfx("chime");
          state.showToast(
            `Task done! ${state.jobTasksDone}/${job.shiftTasks}`,
          );
        }
      });
    });
      ...
      options.push({
        ...
        sub: locked
          ...
          : tooPoor
            ? "Not enough money"
            : `+${a.delta} friendship`,
        disabled: tooPoor || locked,
      });
    }

    const status =
      rel.score >= RELATIONSHIP_FRIEND
        ? "Good friend"
        : rel.score >= 20
          ? "Friendly"
          : rel.score > 0
      ...
      (id) => {
        ...
        if (id.startsWith("tone_")) {
          ...
          delayed(toneDef.durationMs, () => {
            ...
            const raw = Math.round(toneDef.delta * recept);
            const { becameFriend } = state.adjustRelationship(
              npcId,
              raw,
              RELATIONSHIP_FRIEND,
            );
            ...
            if (becameFriend) {
              Audio.sfx("success");
              state.showDialogue(npcId, def.name, friendUnlockLine(npcId));
            } else {
      ...
      (id) => {
        const apply = (
          ...
          delayed(ms, () => {
            ...
            Audio.sfx("success");
            state.showToast(`${def.name} loves it!`);
  ...
  const openShelterMenu = (x: number, y: number) => {
    ...
    menu.show(
      ...
      (id) => {
        ...
        syncPet();
        Audio.sfx("adopt");
        state.showToast(`${pdef.name} joined your family!`, 3200);
  ...
  const handleBuildClick = (tx: number, ty: number, pickedUid?: string) => {
    ...
    if (state.buildTool === "wall") {
      ...
      if (state.walls.has(key)) {
        ...
        if (state.money < 10) {
          Audio.sfx("deny");
          state.showToast("A wall costs $10.");
          return;
        }
  ...
  return {
    ...
    mount(root) {
      ...
      const spawn = { x: Math.floor(state.playerX / TILE), y: Math.floor(state.playerY / TILE) };
      const safeSpawn =
      ...
      hud = new Hud(ui, state, () => quests.getTracker());
      ...
      if (!isContinue) {
        // Let the empty_nest journal land, then advance into Get a Job.
        window.setTimeout(() => {
          if (quests.isActive("empty_nest")) quests.emit("game_started");
        }, 2400);
      } else {
    ...
    update(dt: number) {
      ...
      if (state.mode === "live") {
        for (const id of NEED_IDS) {
          state.needs[id] = clampNeed(state.needs[id] - NEED_DECAY[id] * dt);
        }
      }
      if (state.adoptedPet) {
      ...
      // Hotkeys
      if (justPressed("KeyE") && !dialogue.isOpen()) {
        const t = nearestTarget(40);
        if (t) approach(t);
        else state.showToast("Nothing to use nearby.");
      }
      if (justPressed("KeyB") && !dialogue.isOpen()) toggleBuild();
      if (justPressed("KeyQ") && !dialogue.isOpen()) saveGame();
      if (justPressed("Tab") && state.mode === "build") catalog.toggle();
      if (
        justPressed("KeyR") &&
        state.mode === "build" &&
        state.buildTool === "furniture" &&
      ...
      if (justPressed("Escape") && !dialogue.isOpen()) {
        if (menu.isOpen()) {
          menu.close();
          if (!state.isBusy()) releaseEngagedNpc();
      ...
      for (const npc of npcs) {
        ...
        if (npc.path.length === 0) {
          ...
          const lot = LOTS.find((l) => l.id === def.homeLot)!;
          const goal = {
      ...
      // Pull newly queued dialogue (quests / NPC speech) into the box.
